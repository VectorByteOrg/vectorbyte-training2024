...
$\beta_0 = 13$ ; $\beta_1= 0.9$ ; and $\sigma = 3$. You are asked to predict the population next year if the current population has 90 individuals.   What do you know about $Y$ from the model?
\begin{eqnarray*}
...
<center>![](graphics/sampling1_new.png)</center>

-------------------------------------

<center>![](graphics/sampling2_new.png)</center>
...
-   This is as true for small samples $(n=5)$ as it is for large
    $(n=50)$.
...
<center>![](graphics/linear.png){height="5.5in"}</center>
...
That is, $\displaystyle \bar{X} \rightarrow_{n} \mathcal{N}(\mu, \sigma^2/n)$ , and sample averages tend to be normally
...
\... in terms of its effect on the sampling distributions of the
coefficients
...
$b_1 \sim \mathcal{N}(\beta_1, \sigma^2_{b_1})$.
...
$b_0 \sim \mathcal{N}(\beta_0, \sigma^2_{b_0})$, where
$$
...
1.  Show that:
    ...
    3.  $\text{var}(b_0)  = \sigma^2 \left(\frac{1}{n} + \frac{\bar{X}^2}{(n-1)
            s_x^2} \right)$
...
-   $b_0 \sim t_{n-2}\left(\beta_0, s^2_{b_0}\right)$ and
    $b_1 \sim t_{n-2}\left(\beta_1, s^2_{b_1}\right)$

--------------------------------------

We can use these distributions for drawing conclusions about the
parameters via:
...
<center>![](graphics/prederror_new.png){height="4.5in"}</center>
...
$$ 
We know
$\text{var}(\varepsilon_f) = \sigma^2 \approx s^2$, but what about the fit error?

<center>![](graphics/estimerror_new.png){height="4in"}</center>
...
Replacing $\sigma^2$ with $s^2$ gives the standard error for
$\hat{Y}_f$.
...
<center>![](graphics/spred_new.png){height="4.5in"}</center>
...
<center>![](graphics/fitted.png){height="5in"}</center>
