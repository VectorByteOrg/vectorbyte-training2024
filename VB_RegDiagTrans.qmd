...
We will focus on using graphical methods `r myblue("(plots!)")` to detect violations
...
<center>
`r myblue("Centered at zero")`, $\pm \sim 2.5 \times \sigma$ (here $\sigma = 2$)\
$\rightarrow$ `r mygrn("homoskedastic")`
</center>
...
-   They are almost (close enough) independent
...
<small>
`r myred("(Note: As $n$ gets much bigger, we will expect to see some very rare events (big $\varepsilon_i$) and not get worried unless $|r_i| > 3$ or $4$.)")`
</small> 
...
<center>
![](graphics/distracted_mean_meme.jpg){height="4in"}
</center>
...
A more subtle issue is the normality of the distribution on
$\varepsilon$.
...
If $\mathbb{E}[\log(Y)] = b_0 + b_1 X$, then
$\mathbb{E}[Y] \approx e^{b_0} e^{b_1 X}$.
...
- `r myred("Watch out for over-fitting.")`
  ...
  - but that doesn't mean it will be any good for prediction or
    understanding.
...
-   residuals still aren't normally distributed after taking transforms
    from earlier
...
- Be careful about interpretation after transforming.
  ...
  - You can't use $R^2$ to compare models under different
